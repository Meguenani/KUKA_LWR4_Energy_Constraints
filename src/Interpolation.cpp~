...
/*
    std::cout << "T_v_x : "                << T_v_x <<std::endl;
    std::cout << "T_acc_x : "              << T_acc_x <<std::endl;

    std::cout << "T_v_y : "                << T_v_y <<std::endl;
    std::cout << "T_acc_y : "              << T_acc_y <<std::endl;
    
    std::cout << "T_v_z : "                << (1/(v_z_max)) * (target_posi_z - init_posi_z) * (35/16) <<std::endl;
    std::cout << "T_acc_z : "              << absolute((1/(acc_z_max)) * (target_posi_z - init_posi_z) * (85*sqrt(5)/25)) <<std::endl;
    */
...
double T_acc_angle; 
    ...
    T_v_angle           	=  absolute((1/(v_angle_max)) * (target_rot_angle - init_rot_angle) * (35/16)); 
    T_acc_angle         	=  sqrt(absolute((1/(acc_angle_max)) * (target_rot_angle - init_rot_angle) * (85*sqrt(5)/25))); 
...
double ratioA;
double ratioB;
double s_angle; //Angle équivalent à la rotation due aux quaternions
